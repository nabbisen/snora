...
| 099 | [The sidebar rail is narrower than its buttons](./done/099-the-sidebar-rail-is-narrower-than-its-buttons.md) | **v0.50.0** — Medium. 64 px rail with 16 px uniform padding lays 48 px buttons out 32 wide, icon uncentred; every preset, both variants, ≥ 40 minors. Q-1 (a) derive horizontal padding. Found by orbok (handoff: [`handoffs/099-…`](./handoffs/099-the-sidebar-rail-is-narrower-than-its-buttons/implementation-handoff.md)) |
| 100 | [Dismiss and remove controls use a text glyph where an icon exists](./done/100-dismiss-and-remove-controls-use-a-text-glyph-where-an-icon-exists.md) | **v0.51.0** — Low. Lucide `X` for `Notice` dismiss and `chip` remove under `lucide-icons`; honestly named tooltips (`dismiss_tooltip`, additive `removable_with_tooltip`). Target measured 27 × 28.2, no conformance defect. Requested by orbok (handoff: [`handoffs/100-…`](./handoffs/100-dismiss-and-remove-controls-use-a-text-glyph-where-an-icon-exists/implementation-handoff.md)) |
| 101 | [The toast close button is under the target floor, and still a text glyph](./done/101-the-toast-close-button-is-under-the-target-floor-and-still-a-text-glyph.md) | **v0.51.0** — Medium. Engine toast close measured **25.00 × 23.40** — 0.6 px under snora's mandated 24 px, outside the token-scoped height assertion; plus lucide `X` under `lucide-icons`, rendered by the engine itself. Opened on the owner's instruction from RFC-100's review (handoff: [`handoffs/101-…`](./handoffs/101-the-toast-close-button-is-under-the-target-floor-and-still-a-text-glyph/implementation-handoff.md)) |
| 102 | [The tab bar draws its edges with the wrong primitives, and its state indicator is under the floor](./done/102-the-tab-bar-draws-its-edges-with-the-wrong-primitives.md) | **v0.51.0** — Medium. Underline faked with a shadow (curls), bar border all-sided, hover covers it, sidebar tooltip has no body; **active-tab indicator 2.99:1 on stock Dark**, skipped by the suite. Reported by orbok; item 5 found by the architect (handoff: [`handoffs/102-…`](./handoffs/102-the-tab-bar-draws-its-edges-with-the-wrong-primitives/implementation-handoff.md)) |
| 103 | [The channel register covers variants, not states](./proposed/103-the-channel-register-covers-variants-but-not-states.md) | **proposed** v0.52.0 — REQ-004 (tekstide); chip selected is colour-only; RFC-045 phrase overstates |
| 104 | [Chrome label sizes and toast colours ignore the host](./proposed/104-chrome-label-sizes-and-toast-colours-ignore-the-host.md) | **proposed** v0.52.0 — REQ-002 (tekstide); literal sizes in both variants; plus a build-failing literal-size scan |
| 105 | [Caller text is silently clipped, and two properties are unasserted](./proposed/105-caller-text-is-silently-clipped-and-two-properties-are-unasserted.md) | **proposed** v0.52.0 — REQ-003/006 (tekstide); unbroken text clipped at 288 px; verbatim and bounded work unasserted |
