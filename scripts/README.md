...
| [`check-docs-only.sh`](check-docs-only.sh) | Prints every non-comment, non-blank changed line under `crates/` in a given revision (RFC-092 Part 1A). Empty output is what a "documentation only" / "no behaviour change" claim looks like; any output refutes it — the exact filter that found F-33 and F-34 by hand, after 0.41.1 had already shipped claiming docs-only. | `check-docs-only-claim.sh`; also runnable directly: `scripts/check-docs-only.sh <rev>`. |
| [`check-docs-only-claim.sh`](check-docs-only-claim.sh) | Reads a commit's own message for a `Docs-only: yes` trailer; if present, runs `check-docs-only.sh` against it and fails when the claim doesn't hold. No trailer, no check (RFC-092 Q-1) — this gate has no opinion about commits that make no claim. | The `claims-check` GitHub Actions job, once per commit in a push. |
| [`check-workflows.sh`](check-workflows.sh) | Validates every file under `.github/workflows/` with `actionlint`, pinned to an exact version and verified against a pinned SHA-256 before use — downloaded and cached under `target/actionlint-cache/` if not already on `PATH`. Runnable locally: nobody could parse a workflow file before pushing one, which is how an unquoted step name (`183cc70`) made the entire `ci.yaml` unparseable and GitHub reported zero jobs. Fails loudly, not silently, if `actionlint` cannot be obtained at all. | The `workflow-lint` GitHub Actions workflow (its own file — a broken workflow produces no jobs, so the validator cannot live inside the file it validates), on every push and pull request; also runnable directly: `scripts/check-workflows.sh`. |
