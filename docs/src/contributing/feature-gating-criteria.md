...
| 2. Binary size | 150 KB stripped (`widgets_diff_bytes`) | **52,736 B (~51 KB)** — `binary-size.csv`'s 0.51.0 row, read back after the tag. **35% of a 150 KB bar.** Moved **+1408 B**, but **most of that is subtraction, not widget growth:** the column is `widgets_bytes − engine_bytes`, and `engine_bytes` fell by 1152 B while `widgets_bytes` rose by only 256 B, at the ±256 B noise floor. The engine's only code change is RFC-101's toast close button (a centring box replacing padding), which is the plausible source of the −1152, **not verified**. `design_bytes` rose 2688 B; the design probe renders a sidebar, and RFC-102 gave the sidebar tooltip a body on the design path, which is also plausible and also not verified. None of the four probes renders a tab bar or a toast directly. | **No** |
