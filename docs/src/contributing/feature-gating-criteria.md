...
## Current status (snora 0.51.0, re-derived 2026-09-26, RFC-062)
...
| 2. Binary size | 150 KB stripped (`widgets_diff_bytes`) | **51,328 B (~50 KB)** — `binary-size.csv`'s 0.50.0 row, read back after the tag. **34% of a 150 KB bar.** Moved **+1024 B**, outside the ±256 B floor, with a named cause: 0.50.0's only code change in `snora-widgets` is RFC-099's sidebar (a centring container and derived padding in `build_side_bar`), and `engine_bytes` is byte-identical. **`design_diff_bytes` fell by exactly 1024 B, and that is arithmetic, not a change:** the column is `design_bytes − widgets_bytes`, and `design_bytes` did not move. **Why `design_bytes` stayed flat is unexplained** — the design probe renders a sidebar through the same `build_side_bar` (4 call sites, against the widgets probe's 5), so it should have grown too. 1024 B is a round alignment unit, so section padding absorbing the growth in one binary and not the other is plausible; **not verified**. Recorded so the −1024 is not later read as design getting cheaper. | **No** |
...
| 4. Platform-specific dep | Any system library not already required | None — re-run for 0.51.0: no third-party package moved in `Cargo.lock` since 0.50.0; the only manifest edits are snora's own version pins. `lucide-icons` is now also read by the engine's toast (RFC-101) but was already a dependency of the same feature | **No** |
