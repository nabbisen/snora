...
## Current status (snora 0.36.1, re-derived 2026-08-18, RFC-062)
...
| 2. Binary size | 150 KB stripped (`widgets_diff_bytes`) | **46,336 B (~45 KB)** — `binary-size.csv`'s latest row, 0.36.0 (was 46,464 B at 0.35.0; −128 B). Each release cites the newest row available at cut time; this release's own row appends after the tag | **No** |
