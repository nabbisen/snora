...
|---|---|---|---|
| ~~**RFC-093 Q-1**~~ **— closed 2026-09-12, ruled no** | Adding a non-colour cue to toast intents and notice tones | **The condition was withdrawn as unfireable, and the question ruled on its merits instead.** *"A consumer asks for it"* could not see the case that mattered: **orbok had already built one** (`67c4378`, 2026-06-17, three months before our withdrawal, so independent of it), and a consumer who builds the channel themselves is stronger evidence than one who asks — the condition counted it as silence. Two more of the six were **absent, not "no"**. Ruled rather than re-deferred, because renewing a deferral whose condition was poorly drawn is how the `check-*` manual-to-gate row survived three renewals with its condition true throughout. **Merits:** the gap is already closed by division of labour — `accessibility.md` states it and the channel register's test fails if claim and code diverge — the beneficiary is a future adopter who has not arrived, and three plausible cue designs exist with no requirement to choose between them. Full reasoning in RFC-093 | **No.** Reopens on a different tell: somebody reporting they *expected* the prefab to carry the channel — a documentation failure, not an API gap. Adding a cue stays additive under RFC-036, so waiting costs nothing |
| ~~**Feature-gating indicator 1**~~ **— discharged 2026-09-12** | Assessing compile time at all | **Fired: the measurement exists.** Taken on a developer machine and recorded in `feature-gating-criteria.md`. **~0.2 s marginal, ~150× under the 30 000 ms bar**, so the indicator is **not met** — and the assessment found two things worth more than the number. **The procedure printed on that page measured a no-op** (`cargo clean -p <pkg>` does not reach the release profile, so the timed release build returned in 393 ms with nothing rebuilt) — RFC-052's own defect, in the procedure for assessing an indicator, six minors after RFC-052 fixed it in the script. And the **threshold's magnitude fits the cold-closure quantity (29 523 ms measured) rather than the marginal one**, which per-widget gating cannot reduce at all | **No.** One thing left, and it is the owner's: restate the threshold against the quantity the command measures, or change the command to match. Recording both numbers was the measurement's job; rewriting a threshold is a design decision |
...
| ~~**Multi-platform CI**~~ **— closed 2026-09-06, not warranted** | Extending CI beyond `ubuntu-latest` | **Fired at the 0.46.0 cut and answered no.** The owner asked what was actually left to do given snora has no OS-dependent code, and checking settled it: `cargo update --dry-run` on our own workspace **does not move `windows` or `gpu-allocator` at all** — our graph holds `windows` at 0.58.0/0.62.2 and a full re-resolution leaves both untouched. **A Windows `unpinned-build` would not have caught arama's bug.** | **Nothing.** The mitigation already shipped: the 0.41→0.42 guide tells consumers to re-resolve and says their own CI is the only thing that sees this class |
| ~~**The other 32 unswept gate rows**~~ **— discharged 2026-09-12, condition fired** | Re-deriving the rows of `api-freeze-review.md` that RFC-094 left alone | **Fired exactly as written.** The condition was *one more found stale by other work*; **D-7** was the second — its row claimed a per-minor scope review and named v0.24 as its last clean point, 23 minors during which `snora-widgets/src` moved +1,897/−788 across 28 files. Sweep performed: 19 rows dated, four gates moved (D-3, D-4, D-7 closed; the feature-flag row re-ticked), two release-hygiene rows closed on measurements that accumulated after they were opened, and one real defect found and fixed — `reference/vocabulary.md` claimed to list *every public enum in snora-core* and had omitted two since 0.39.0. See the register's own **Sweep record** | **No.** Judgement-only rows were left undated deliberately: under RFC-094's Q-2 convention an undated row already says nobody has re-derived it, and dating them to look complete would destroy the register's only signal |


**Why it was closed, since the reasoning is worth more than the row.** The
recommendation to add Windows and macOS to `unpinned-build` was mine, and it did
not survive being checked.

1. **`ci.yaml` on Windows was never the answer** — our CI builds from the
   committed `Cargo.lock`, and arama's failure was a *re-resolution*.
   `unpinned-build` is the only job that runs `cargo update`.
2. **`unpinned-build` on Windows is not the answer either.** Checked
   2026-09-06: `cargo update --dry-run` on this workspace leaves `windows` and
   `gpu-allocator` untouched. Our requirement set still holds `windows` at 0.58;
   arama's had lost the requirements doing that. **We cannot reproduce the bug
   because we are not the consumer.**
3. **snora has no OS-dependent code** — zero `cfg(target_os)`, `cfg(windows)`,
   `cfg(unix)`, `cfg(target_family)` in `crates/`.

**The failure class is consumer-graph-shaped, not framework-graph-shaped.** It
depends on what *else* a consumer depends on, so no amount of platform CI on our
side reaches it. The correct mitigation is telling consumers, which the
0.41→0.42 guide already does.

Recorded because the wrong version of this change is plausible enough that
