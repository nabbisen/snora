...
| **The other 32 unswept gate rows** | Re-deriving the rows of `api-freeze-review.md` that RFC-094 left alone. Its Q-1 ruled test-backed rows only; 33 were listed and not swept | **One more of them is found stale by other work.** The 2026-09-12 audit found the first — *"Type-names audit: complete as of v0.17.0"*, 29 minors old — which is one instance, not a pattern. A second makes it a pattern and earns the sweep | **No.** The first instance is dated in place rather than fixed, per RFC-094's Q-2 convention: an undated row has never been re-derived, and that is now visible without a sweep |
