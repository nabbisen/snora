...
| 9b. **Compile-time** trend monitored (≥2 data points) | ⬜ **open, and its clock reset again (RFC-052) — third methodology discontinuity after RFC-043 and RFC-044.** The pre-existing noise problem is unresolved: four rows spread 21.5% (`check_workspace_ms`), 24.8% (`build_widgets_ms`) and 27.1% (`example_hello_ms`), and 0.28.0 → 0.28.1 changed **no code at all** — documentation only — yet moved those figures **+11.3%, +9.5%, +7.7%**. On top of it, `measure-compile-time.sh`'s per-measurement clean (`cargo clean -p <pkg>`) was found to reach the dev profile only — `build_engine_only_ms` and `build_widgets_design_ms` (both `release`-profile) rode on artifacts left behind by an earlier measurement in the same run rather than measuring a rebuild, confirmed via `Compiling`-line evidence, not a timing delta. Fixed by cleaning all three profiles (dev, `release`, `release-baseline`) unconditionally before every measurement. **This is not progress toward closure — it moves further away**, deliberately: rows before and after the fix are not comparable for those two columns (RFC-041 N-1, no back-fill), so the ≥2-comparable-post-fix-data-points clock restarts from zero for them. Do not read the fix landing as satisfying any part of this gate. Re-satisfy once the measurement is noise-controlled (e.g. repeated runs reduced to a median, or a runner-load-independent metric) **and** ≥2 post-RFC-052 data points exist for every column. See `build-cost-budget.md`'s RFC-052 note for the full evidence. |
