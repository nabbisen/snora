...
| 1. One iced major upgrade completed and lived on ≥1 minor | ⬜ — externally gated. **When it fires, re-check the test harness before trusting a green suite:** `.cargo/config.toml` sets `ICED_TEST_BACKEND = "tiny-skia"` (RFC-099 R-1), which relies on `iced_test` 0.14 reading that variable and on wgpu's headless constructor returning before it constructs its `Instance` when the hint is not `"wgpu"`. If either changes, the line stops working **without an error** and parallel simulator tests can crash with SIGSEGV in the Vulkan loader again. **To re-check, use a probe that cannot pass by accident:** run a simulator test with `ICED_TEST_BACKEND=no-such-backend` exported — it **must panic** with *"Create new headless renderer"*, because no renderer accepts that hint. If it passes instead, the variable is no longer being read and the protection is gone. (Do not use `ICED_TEST_BACKEND=wgpu` as the probe: an `iced_test` that ignored the variable would also default to wgpu and crash, so a crash there proves nothing.) Recorded here because a gate row is what an iced upgrade will actually read. |
